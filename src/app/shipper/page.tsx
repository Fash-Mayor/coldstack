...
import {
  getShipperTrips,
  getTripDetails,
} from "@/app/actions/shipper";
import { TripCard } from "@/components/carrier/trip-card";
import { POCModal } from "@/components/carrier/poc-modal";
import type { ShipperTrip, TripPhotoRow } from "@/app/actions/shipper";

export default function ShipperPage() {
  const [trips, setTrips] = useState<ShipperTrip[]>([]);
  const [isInitialLoad, setIsInitialLoad] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTrip, setSelectedTrip] = useState<ShipperTrip | null>(null);
  ...
  async function loadTrips() {
    try {
      setError(null);
      const data = await getShipperTrips();
      setTrips(data);
    } catch (err) {
  ...
  async function handleTripCardClick(trip: ShipperTrip) {
    if (trip.status === "completed" || trip.status === "cancelled") {
      try {
        setError(null);
        const { trip: fullTrip, photos } = await getTripDetails(trip.id);
        setSelectedTrip(fullTrip as ShipperTrip);
              ...
              {pendingActiveTrips.length === 0 ? (
                <div className="bg-white border-2 border-dashed border-slate-300 rounded-xl p-12 text-center">
                  <p className="text-slate-600 text-sm">
                    No pending or active trips yet. Your shipments will appear
            ...
            <section>
              ...
              {completedTrips.length === 0 ? (
                <div className="bg-white border-2 border-dashed border-slate-300 rounded-xl p-10 text-center">
                  <p className="text-slate-600 text-sm">
                    No completed trips yet. Completed trips will appear here
                    after delivery.
                  </p>
                </div>
              ) : (
